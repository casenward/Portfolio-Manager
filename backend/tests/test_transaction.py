"""Test suite for the portfolio_manager package's operations.
...
class TestResolveYfTicker:
    def test_plain_ticker_passthrough(self):
        assert tickers.resolve_yf_ticker("AAPL") == "AAPL"

    def test_lowercase_is_uppercased(self):
        assert tickers.resolve_yf_ticker("aapl") == "AAPL"

    def test_whitespace_is_stripped(self):
        assert tickers.resolve_yf_ticker("  msft  ") == "MSFT"

    def test_override_b_maps_to_gold(self):
        # "B" is special-cased to Barrick Mining's yfinance symbol.
        assert tickers.resolve_yf_ticker("B") == "GOLD"
    ...
    def test_slash_ticker_is_dashed(self):
        ...
        assert tickers.resolve_yf_ticker("BRK/B") == "BRK-B"
...
class TestLoadHoldings:
    ...
    def test_basic_load_normalizes_account_and_ticker(self, tmp_path):
        ...
        holdings = repository.load_holdings(path)
    ...
    def test_missing_file_raises(self, tmp_path):
        with pytest.raises(FileNotFoundError):
            repository.load_holdings(tmp_path / "nope.json")
...
class TestPortfolioValue:
    def test_basic_sum(self):
        ...
        total, missing = portfolio.portfolio_value(positions, prices)
        assert total == 10 * 100.0 + 5 * 200.0
        assert missing == []

    def test_empty_positions_returns_zero(self):
        total, missing = portfolio.portfolio_value([], {})
        assert total == 0.0
        assert missing == []
    ...
    def test_ticker_absent_from_prices_dict_counts_as_missing(self):
        ...
        total, missing = portfolio.portfolio_value(positions, {})
        assert total == 0.0
        assert missing == ["AAPL"]

    def test_fractional_shares(self):
        positions = [{"ticker": "AAPL", "yf_ticker": "AAPL", "shares": 0.5}]
        total, _ = portfolio.portfolio_value(positions, {"AAPL": 100.0})
        assert total == 50.0
...
class TestFormatDollars:
    def test_thousands_separator_and_two_decimals(self):
        assert formatting.format_dollars(1234567.891) == "$1,234,567.89"

    def test_zero(self):
        assert formatting.format_dollars(0) == "$0.00"

    def test_negative_value(self):
        assert formatting.format_dollars(-500.5) == "$-500.50"

    def test_rounding(self):
        assert formatting.format_dollars(9.995) == "$10.00" or formatting.format_dollars(9.995) == "$9.99"
        # banker's rounding / float repr can go either way; just confirm it doesn't crash
        # and produces a 2-decimal dollar string
        result = formatting.format_dollars(9.995)
...
def isolate_transactions_file(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(transactions, "TRANSACTIONS_PATH", tmp_path / "transactions.csv")
    yield
...
class TestBuyStock:
    def test_buy_increases_shares_and_deducts_cash(self, holdings):
        trading.buyStock(100.0, 2, holdings, "AAPL", "traditional")
...
class TestSellStock:
    def test_sell_decreases_shares_and_increases_cash(self, holdings):
        holdings["cash"] = 0.0
        trading.sellStock(100.0, 3, holdings, "AAPL", "traditional")
    ...
    def test_sell_all_shares_removes_position(self, holdings):
        trading.sellStock(100.0, 10, holdings, "AAPL", "traditional")
        holdings_tickers = [p["ticker"] for p in holdings["traditional"]]
    ...
    def test_sell_more_than_owned_raises_and_does_not_mutate(self, holdings):
        with pytest.raises(ValueError, match="Not enough shares"):
            trading.sellStock(100.0, 999, holdings, "AAPL", "traditional")
        assert holdings["traditional"][0]["shares"] == 10.0

    def test_sell_unknown_ticker_raises(self, holdings):
        with pytest.raises(ValueError, match="not found"):
            trading.sellStock(100.0, 1, holdings, "GOOG", "traditional")

    def test_sell_wrong_account_raises_even_if_ticker_exists_elsewhere(self, holdings):
        with pytest.raises(ValueError, match="not found"):
            trading.sellStock(100.0, 1, holdings, "TSLA", "traditional")

    def test_sell_logs_transaction(self, holdings, tmp_path):
        trading.sellStock(100.0, 2, holdings, "AAPL", "traditional")
        rows = list(csv.DictReader((tmp_path / "transactions.csv").open()))
        assert len(rows) == 1
...
class TestDayChange:
    def test_sums_change_across_accounts(self, holdings):
        ...
        assert portfolio.dayChange(prices, previous, holdings) == 140.0
    ...
    def test_skips_missing_current_or_previous_price(self, holdings):
        ...
        previous = {"AAPL": 100.0, "TSLA": 190.0}
        assert portfolio.dayChange(prices, previous, holdings) == 100.0
    ...
    def test_cash_key_is_ignored(self, holdings):
        ...
        assert portfolio.dayChange(prices, previous, holdings) == 50.0
    ...
    def test_uses_yf_ticker_not_display_ticker(self):
        ...
        assert portfolio.dayChange(prices, previous, holdings) == 20.0

    def test_empty_holdings_returns_zero(self):
        assert portfolio.dayChange({}, {}, {"cash": 100.0}) == 0.0
    ...
    def test_fractional_shares(self):
        ...
        assert portfolio.dayChange({"AAPL": 110.0}, {"AAPL": 100.0}, holdings) == 5.0
...
class TestLogTransaction:
    def test_writes_header_on_first_write(self, tmp_path):
        ...
        transactions.log_transaction("buy", 100.0, 2, "AAPL", "traditional")
    ...
    def test_appends_without_duplicate_header(self, tmp_path):
        transactions.log_transaction("buy", 100.0, 1, "AAPL", "traditional")
        transactions.log_transaction("sell", 50.0, 1, "AAPL", "traditional")
